...
fn run() -> Result<(), String> {
    ...
    let delete_after_upload = param_bool("delete_after_upload", false);
    ...
    // 上传成功后，根据用户配置决定是否删除本地文件
    // After successful upload, delete the local file if the user enabled the option.
    // 输出 DELETE_INPUT 协议行请求主程序删除输入文件及其 meta 文件；
    // 否则输出 OUTPUT:{path} 将视频路径传递给流水线后续模块。
    if delete_after_upload {
        println!("DELETE_INPUT");
        eprintln!(
            "Upload succeeded, requesting deletion of local file: {}",
            input.display()
        );
    } else {
